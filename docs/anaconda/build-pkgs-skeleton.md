...
2. Use the `ls` command on OS X or Linux or the `dir` command on Windows to verify the meta.yaml was created. 
...
5. Now verify that Click installed successfully:
...
This works only for pure Python packages where
